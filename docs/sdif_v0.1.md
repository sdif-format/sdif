...
8. Expand or normalize aliases deterministically.
9. Normalize booleans to `true` and `false`.
10. Normalize null to `null`.
11. Normalize dates and date-times.
12. Normalize numeric representation.
13. Escape strings consistently.
14. Sort relations when order is not significant.
15. Sort table rows when order is not significant.
16. Preserve order when the schema declares order significant.
