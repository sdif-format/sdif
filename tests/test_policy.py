...
def test_policy_include_depth(tmp_path):
    ...
    policy = Policy(allow_includes=True, allowed_include_paths=frozenset([tmp_path]), max_include_depth=2)
    with pytest.raises(PolicyError) as excinfo:
        parse_file(f1, policy=policy)
...
def test_policy_expanded_bytes(tmp_path):
    ...
    policy = Policy(allow_includes=True, allowed_include_paths=frozenset([tmp_path]), max_expanded_bytes=30)
    with pytest.raises(PolicyError) as excinfo:
        parse_file(f1, policy=policy)
